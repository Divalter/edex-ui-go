 ...
 *  - Runs as an ES module on top of the Node/Electron replacements of
 *    src/host (Go backend reached through the Wails IPC).
 *  - Settings, shortcuts and window state come from the backend at startup;
...
// Local files are served by the backend instead of file:// URLs
window._encodePathURI = uri => {
    return window.edexHost.fileURL(uri);
};
...
// Inline event handlers and the classes expect these as globals, like the
// top-level constants of the original non-module script.
Object.assign(window, {path, fs, electron, remote, ipc, settingsDir, themesDir, keyboardsDir, fontsDir, settingsFile, shortcutsFile, lastWindowStateFile});
...
// Load CLI parameters
window.settings.nointroOverride = boot.nointroOverride;
window.settings.nocursorOverride = boot.nocursorOverride;

const readJSON = async file => JSON.parse(await fs.promises.readFile(file));
const ipcInvoke = channel => new Promise(resolve => {
    ipc.once(channel, (e, value) => resolve(value));
    ipc.send(channel);
});
...
window._loadTheme = theme => {
    ...
    // Load fonts
    let fontURL = name => window.edexHost.fileURL(path.join(fontsDir, name.toLowerCase().replace(/ /g, '_')+'.woff2'));
    let mainFont = new FontFace(theme.cssvars.font_main, `url("${fontURL(theme.cssvars.font_main)}")`);
    let lightFont = new FontFace(theme.cssvars.font_main_light, `url("${fontURL(theme.cssvars.font_main_light)}")`);
    let termFont = new FontFace(theme.terminal.fontFamily, `url("${fontURL(theme.terminal.fontFamily)}")`);
...
{
    let theme = await ipcInvoke("getThemeOverride");
    if (theme !== null) {
        window.settings.theme = theme;
        window.settings.nointroOverride = true;
    }
    _loadTheme(await readJSON(path.join(themesDir, window.settings.theme+".json")));
}
// Same for keyboard override/hotswitch
{
    let layout = await ipcInvoke("getKbOverride");
    if (layout !== null) {
...
// On-screen keyboard visibility (eDEX-UI-GO addition)
window.applyKeyboardVisibility = () => {
    document.documentElement.classList.toggle("keyboard_hidden", window.settings.hideKeyboard === true);
};
window.applyKeyboardVisibility();
...
// A proxy object forwarding systeminformation calls to the Go backend (internal/sysinfo)
    ...
    window.si = new Proxy({}, {
        ...
        get: (target, prop, receiver) => {
            return function(...args) {
                ...
                return window.edexHost.call("si", prop, ...args).then(res => {
                    if (callback) {
                        callback(res);
                    }
                    return res;
...
async function displayLine() {
    ...
    if (!bootLog) {
        bootLog = (await (await fetch("assets/misc/boot_log.txt")).text()).split('\n');
    }
    let log = bootLog;
    ...
    switch(true) {
        case i === 2:
            bootScreen.innerHTML += `eDEX-UI-GO Kernel version ${electron.remote.app.getVersion()} boot at ${Date().toString()}; root:xnu-1699.22.73~1/RELEASE_X86_64`;
        case i === 4:
...
async function displayTitleScreen() {
    ...
    bootScreen.innerHTML = "<h1>eDEX-UI-GO</h1>";
...
async function initUI() {
    ...
    window.keyboard = new Keyboard({
        layout: await readJSON(path.join(keyboardsDir, settings.keyboard+".json")),
        container: "keyboard"
    });
    ...
    window.term[0].term.writeln("\x1b[1m"+`Welcome to eDEX-UI-GO v${electron.remote.app.getVersion()} - ${boot.runtime}`+"\x1b[0m");
    window.term[0].term.writeln(`Based on eDEX-UI by Gabriel 'Squared' SAILLARD - https://github.com/GitSquared/edex-ui`);
...
window.remakeKeyboard = async layout => {
    let layoutData = await readJSON(path.join(keyboardsDir, (layout || settings.keyboard)+".json"));
...
// Show or hide the on-screen keyboard and remember the choice (eDEX-UI-GO addition)
window.toggleKeyboard = () => {
    window.settings.hideKeyboard = !(window.settings.hideKeyboard === true);
    window.applyKeyboardVisibility();
    readJSON(settingsFile).then(saved => {
        saved.hideKeyboard = window.settings.hideKeyboard;
        fs.writeFileSync(settingsFile, JSON.stringify(saved, "", 4));
    }).catch(e => console.warn("Cannot save hideKeyboard setting", e));
    setTimeout(() => {
...
window.openSettings = async () => {
    ...
    (await fs.promises.readdir(keyboardsDir)).forEach(kb => {
        if (!kb.endsWith(".json")) return;
        kb = kb.replace(".json", "");
        if (kb === window.settings.keyboard) return;
        keyboards += `<option>${kb}</option>`;
    });
    (await fs.promises.readdir(themesDir)).forEach(th => {
        ...
        html: `<table id="settingsEditor">
                    ...
                    <tr>
                        ...
                        <td><select id="settingsEditor-hideKeyboard">
                            <option>${window.settings.hideKeyboard === true}</option>
                            <option>${!(window.settings.hideKeyboard === true)}</option>
                        </select></td>
                    </tr>
                    <tr>
                ...
                <br>`,
        buttons: [
            {label: "Open in External Editor", action:`electron.shell.openPath(${window._escapeHtml(JSON.stringify(settingsFile))});electronWin.minimize();`},
            {label: "Save to Disk", action: "window.writeSettingsFile()"},
            {label: "Reload UI", action: "window.location.reload(true);"},
            {label: "Restart eDEX", action: "electron.remote.app.relaunch();"}
        ]
    }, () => {
...
window.openShortcutsHelp = () => {
    ...
    const shortcutsDefinition = {
        ...
        "KB_TOGGLE": "Show or hide the on-screen keyboard.",
    ...
    new Modal({
        ...
        html: `<h5>Using either the on-screen or a physical keyboard, you can use the following shortcuts:</h5>
                ...
                <br>`,
        buttons: [
            {label: "Open Shortcuts File", action:`electron.shell.openPath(${window._escapeHtml(JSON.stringify(shortcutsFile))});electronWin.minimize();`},
            {label: "Reload UI", action: "window.location.reload(true);"},
        ]
    }, () => {
        window.keyboard.attach();
...
if (!window.shortcuts.some(cut => cut.action === "KB_TOGGLE")) {
    window.shortcuts.push({ type: "app", trigger: "Ctrl+Shift+Alt+K", action: "KB_TOGGLE", enabled: true });
}
