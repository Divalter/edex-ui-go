 ...
 *  - Only the client role is kept: the server role (node-pty + ws) is internal/terminal in Go.
...
class Terminal {
    constructor(opts) {
        if (opts.role === "client") {
            ...
            this.socket = window.edexHost.openTTY(sockPort);
            this.socket.onopen = () => {
            ...
            this.socket.addEventListener("message", e => {
                ...
                let ips = text.match(/((25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)/g);
            ...
            this.clipboard = {
                copy: () => {
                    ...
                    electron.remote.clipboard.writeText(this.term.getSelection());
                    this.term.clearSelection();
                    this.clipboard.didCopy = true;
                },
                paste: () => {
                    Promise.resolve(electron.remote.clipboard.readText()).then(text => this.write(text || ""));
